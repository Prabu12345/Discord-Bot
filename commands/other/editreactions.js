...
module.exports = class CatCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      aliases: ['editreact'],
      group: 'other',
      memberName: 'editreactions',
      description: 'To edit reactions role',
      throttling: {
        usages: 2,
        duration: 10
      },
  ...
  async run(message, client, { reaction }) {
    if(reaction == '') return;
        // Check if the message exists.
        const { channel, author } = message;
        try {
            let fetchedMessage = channel.messages.cache.get(reaction) || await channel.messages.fetch(reaction);
            if(!fetchedMessage) 
                ...
                if(msgModel) {
                    ...
                    try {
                        ...
                        if(choice.content === "add") {
                            ...
                            let collectorResult = await handleCollector(fetchedMessage, author, channel, msgModel, reaction);
